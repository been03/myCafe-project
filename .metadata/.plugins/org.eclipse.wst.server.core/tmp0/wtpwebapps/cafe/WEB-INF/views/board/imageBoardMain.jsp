...
<style>
	...
	.image_board_ul li:first-child, .image_board_ul li:nth-child(6){
		margin-left: 7px;
	}
	.image_board_ul li div{
		text-align: left;
    	padding-left: 15px;
	}
</style>
           	 ...
           	 <c:forEach var="list" items="${articles}" begin="${begin }" end="${end-1 < 0 ? 0 : end-1 }" varStatus="status"> 
           		<li>
           			<c:choose>
       					...
       					<img src="${list.imageName }" width="150px" height="150px">
       				</a>
           			<div>
           				<c:choose>
            				<c:when test="${(member.role!='ADMIN' && member.memberLevel >= category.ctgReadLevel && category.ctgReadLevel != 0) || member.role == 'ADMIN' || (category.ctgReadLevel == 7 && member.memberLevel >=2)}">
	           				...
	           				<c:otherwise>
           						...
           						<span class="article_title_span" style="margin: 0; font-weight: bold;">
           							${list.articleTitle }
           						</span>
           					</a> 
           				<p>${list.nickName }</p>
      						...
      						<span>
           						<c:if test="${member.role == 'ADMIN' }">
				                	<c:choose>
						                ...
						                <c:otherwise>
						                	<a style="color:red" href="javascript:alert('본인이 작성한 글만 수정할 수 있습니다!');">수정</a>
						                	<button id="deletebutton" class="delete_button" onClick="deleteArticle(${list.articleId})">삭제</button>
						                </c:otherwise>
							        </c:choose>
						       	</c:if>
         ...
         <ul class="page_nav_ul">
         	...
         	<c:choose>
	         	<c:when test="${pageCount/10 > curBlock+1 }">
	         		<li class="page_nav_li">
	         			<a href="boardMain.do?boardId=${boardId }&curBlock=${curBlock+1 }&pageNum=${pageEnd}">${pageCount/10 }&gt;</a>
	         		</li>
	         	</c:when>
	         	<c:otherwise>
	         		<li class="page_nav_li">
