...
<style>
	...
	<section class="article_write_section">
		<form class="write_form" name="write_form" method="post" action="${contextPath}/member/submit.do" id="frm" enctype="multipart/form-data">
			<c:choose>
				<c:when test="${not empty article }">
					<div class="boardinfo_wrap">
						...
						<select id="boardName" name="boardName" disabled>
							...
							<c:if test="${member.role == 'ADMIN' }">
								<option value="1" <c:out value="${'공지사항' == article.articleBoard ? 'selected' : '' }"/> >공지사항</option>
								<c:forEach var="list" items="${boardList }">
									<option value="${list.boardId }" <c:out value="${list.boardName == article.articleBoard ? 'selected' : '' }"/>>${list.boardName }</option>
								</c:forEach>
							</c:if>
						</select>
						<label for="boardName">말머리 (하나만 선택 가능합니다)</label>
						<select id="articleHead" name="articleHead" disabled>
							<c:choose>
								<c:when test="${member.role == 'ADMIN'}">
									<c:if test="${article.nickName != 'ADMIN' }">
										<option value="답글" selected>답글</option>
									</c:if>
									<c:if test="${article.nickName == 'ADMIN' }">
					...
					<label for="articleTitle">제목<input id="articleTitle" type="text" name="articleTitle" value="${article.articleTitle }" readOnly></label>
			    	<label for="articleWriter">작성자<input id="articleWriter" type="text" name="articleWriter" value="${member.nickName }" readOnly></label>
			    	<label for="articleDate">작성일자<input id="articleDate" type="text" name="articleDate" value="${date }" readOnly> </label>
			    	<input type="hidden" name="boardName" value="${boardInfo.boardName }">
				...
				<c:otherwise>
					<div class="boardinfo_wrap">
						...
						<select id="boardName" name="boardName">
							<c:if test="${member.role != 'ADMIN' }">
								<c:forEach var="list" items="${boardListUser }">
									<option value="${list.boardId }" <c:out value="${boardInfo.boardName == list.boardName ? 'selected' : '' }"/>>${list.boardName }</option>
								</c:forEach>
							</c:if>
							<c:if test="${member.role == 'ADMIN' }">
								<option value="1" <c:out value="${'공지사항' == boardInfo.boardName ? 'selected' : '' }"/> >공지사항</option>
								<c:forEach var="list" items="${boardList }">
					...
					<label for="articleTitle">제목<input id="articleTitle" type="text" name="articleTitle" value="" required placeholder="제목을 입력하세요" maxLength="50"></label>
				    <label for="articleWriter">작성자<input id="articleWriter" type="text" name="articleWriter" value="${member.nickName }" readOnly></label>
				    <label for="articleDate">작성일자<input id="articleDate" type="text" name="articleDate" value="${date }" readOnly> </label>
				    <input type="hidden" name="boardId" value="${boardInfo.boardId }">
		    ...
		    <c:if test="${empty article }">
			    <div class="tag_wrap">
					...
					<div id="article_tag_div" class="article_tag_div">태그: </div>
				</div>
			</c:if>
			<c:if test="${not empty article }">
				<div class="tag_wrap">
			    	<label for="tag">태그 추가하기<input id="tag" placeholder="답글은 태그를 추가할 수 없습니다" disabled maxLength="15" type="text" name="tag"></label>
					<input type="button" class="add_tag_button" onClick="addArticleTag()" value="추가" disabled>
						...
						<c:forEach items="${tagList }" var="list" varStatus="status">
							<c:if test="${list != 'NONE_TAG' && list != '[NONE_TAG]' }">
								<span id='articleTag${status.index+1}' class='article_tag_span'>#${list }
									<i style='margin-left:5px; color:red' class='fas fa-times' onClick='javascript:alert("답글쓰기에서 태그수정을 할 수 없습니다!");'></i>
								</span>
		    ...
		    <span class="write_icon_span"><i class="fas fa-pen-square"></i></span><input type="button" class="save_button" id="savebutton" value="작성하기" />
		</form>
	</section>
	<script type="text/javascript">
		$(function(){
			...
			//쓰기 권한보다 낮을때
		    if (check < "${category.ctgWriteLevel}" && "${member.role}" != 'ADMIN'){
		    	$(".article_write_section").css("visibility","hidden");
		    	//alert("회원님의 등급이 낮습니다!");
		    	window.location.href="${contextPath }/member/levelCheck.do?boardId=${boardInfo.boardId}";
		    //로그인 계정이 관리자 계정이 아니고 쓰기권한이 관리자레벨일 때
		    }else if (("${member.role}" != 'ADMIN' && "${category.ctgWriteLevel}" == 0)){
		    	$(".article_write_section").css("visibility","hidden");
		    	//alert("회원님의 등급이 낮습니다!");
		    	window.location.href="${contextPath }/member/levelCheck.do?boardId=${boardInfo.boardId}";
		    }else if ("${boardInfo.ctgId}" == 0 && "${member.role}" != 'ADMIN' ){
		    	$(".article_write_section").css("visibility","hidden");
				//alert("관리자만 사용가능한 게시판입니다!");
				window.location.href="${contextPath }/member/levelCheck.do?boardId=${boardInfo.boardId}";
		    }
		    ...
		    $("#savebutton").click(function(){
		        ...
		        form.articleHead.disabled = false;
